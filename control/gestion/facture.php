    ...
    if($mode_reglement == 3){
        ...
        while($products = mysql_fetch_assoc($sql_article)) {
            foreach($products as $k => $product){
                $params["L_PAYMENTREQUEST_0_NAME$k"] = $product[0]["nom_article"];
                $params["L_PAYMENTREQUEST_0_DESC$k"] = '';
                $params["L_PAYMENTREQUEST_0_AMT$k"] = $product[0]["total_ligne"];
                $params["L_PAYMENTREQUEST_0_QTY$k"] = $product[0]["qte"];
            }
            print_r($products);
        }
